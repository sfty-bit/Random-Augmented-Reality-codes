...
while True:
	...
	if len(corners) > 0:
		...
		for (markerCorner, markerID) in zip(corners, ids):
			...
			rvec_list_all, tvec_list_all, _objPoints = aruco.estimatePoseSingleMarkers(corn, marker_size, camera_matrix, camera_distortion)
			...
			for i in range(len(corn)):
				cv2.putText(frame, str(tvec_list_all[i]),
